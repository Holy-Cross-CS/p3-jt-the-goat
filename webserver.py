...
topicsLikes = [2, 3, 0]
topicsCt = [0, 0, 0]
tweetList = [["love it #rollTide", "they suck"], ["love it", "third world"], ["hate em", "champions"]]
...
def handle_http_get_hello(req, conn):
    ...
    if req.path.startswith("/hello?"):
        ...
        start = query_string.find("username=") + len("username=")

        end = query_string.find("&", start)
        if end == -1:
            end = len(query_string)

        name = query_string[start:end].replace("+", " ")
        start = query_string.find("favorite=") + len("favorite=")

        end = query_string.find("&", start)
        if end == -1:
            end = len(query_string)

        favcolor = query_string[start:end]
    else:
        name = "Earthling"
        favcolor = "black"

    if favcolor not in [ "black", "crimson", "navy", "purple", "tomato", "teal", "wheat", "pink", "green",
    "yellow", "orange", "blue", "gold" ]:
        favcolor = None
    ...
    msg = " "
    msg += '<html>'
    msg += '<head><title> Hello Page </title>'
    msg += '<style>'
    msg += 'body{'
     #cycling through background image
     #makes it hard to read but i like
    if counter % 4 == 1:
        msg += 'background-image: url("dobby.jpeg");'
        msg += 'background-size: cover;'
        msg += 'background-position: center;'
        msg += 'background-repeat: no-repeat;'
        msg += 'background-attachment: fixed;'
    elif counter % 4 == 2:
        msg += 'background-image: url("myPrecious.jpg");'
        msg += 'background-size: cover;'
        msg += 'background-position: center;'
        msg += 'background-repeat: no-repeat;'
        msg += 'background-attachment: fixed;'
    elif counter % 4 == 3:
        msg += 'background-image: url("spider.jpg");'
        msg += 'background-size: cover;'
        msg += 'background-position: center;'
        msg += 'background-repeat: no-repeat;'
        msg += 'background-attachment: fixed;'
    ...
    msg += f'<p>The current date is {time.strftime("%A, %d %b %Y %H:%M:%S", time.localtime())}</p>'
    msg += f'<p>You have visited this page {counter} times before.</p>'
    ...
    msg += '</ul>'
    msg += f'<li><a href="http://{server_host}:{server_port}/hello"> - this page\n</a></li>'
    msg += f'<li><a href="http://{server_host}:{server_port}/status"> - status and statistics\n</a></li>'
    msg += f'<li><a href="http://{server_host}:{server_port}/quote"> - a random quote\n</a></li>'
    msg += f'<li><a href="http://{server_host}:{server_port}/welcome.html"> - a welcome page\n</a></li>'
    msg += f'<li><a href="http://{server_host}:{server_port}/index.html"> - some fancy web page\n</a></li>'
    msg += '</ul>'
    msg += '<form method="GET" action="/hello">'
    msg += '<p>Enter your name: <input type="text" name ="username"\n></p>'
    msg += '<p>Favorite color: <select name="favorite"\n></p>'
    ...
    msg += '<option value="pink">A nice Salmon Pink</option>'
    msg += '<option value="green">A nice Power Rangers Green</option>'
    msg += '<option value="yellow">A nice Spongebob Yellow</option>'
    msg += '<option value="orange">A nice Flinstone Orange</option>'
    msg += '<option value="blue">A nice Sonic Blue</option>'
    msg += '<option value="gold">A nice Super Saiyan Gold</option>'
    msg += '</select>'
    msg += '    <input type="submit" value="Not Me!"></form></li>'

    if favcolor == "gold":
        msg += '<img src="SuperSaiyan.jpg" height="300" alt="This is the best color combo" title="SS3">'

    msg += '<p>Is this page interactive now?\n</p>'
    msg += '<label for="yes">Yes</label>'
    msg += '<input type="checkbox" id="yes" name="interactive" value="yes">'
    msg += '<label for="no">No</label>'
    msg += '<input type="checkbox" id="no" name="interactive" value="no">'
    msg += '</body>'
    msg += '</html>'
...
#handle_http_get_whoami(conn)
def handle_http_get_whoami(conn):
    log("Handling http get identity request")

    msg = "Your Super Secret Information:\n"
    msg += "\n"
    msg += "Identity:\n"
    client_ip = conn.client_addr
    birth = time.localtime(stamp)
    birth = time.strftime('%Y-%m-%d %H:%M:%S', birth)
    
    msg += f"Your IP is: {client_ip}\n"
    msg += f"Your Socket is: {server_port}\n"
    msg += f'"Date of Birth at: {birth}\n"'
    msg += f"Your Cookie Jar Holds: Nothing I ate them all\n"
...
#attempt to manage mime types
mime_types = {
    ".html": "text/html", ".htm": "text/html", ".txt": "text/plain", ".css": "text/css", 
    ".js": "text/javascript", ".jpg": "image/jpeg", ".jpeg": "image/jpeg", 
    ".png": "image/png", ".gif": "image/gif", ".pdf": "application/pdf", ".xls": "application/vnd.ms-excel", 
    ".ppt": "application/vnd.ms-powerpoint", ".mp4": "video/mp4", ".mp3": "audio/mpeg", 
    ".midi": "audio/x-midi", ".doc": "application/msword"
}
...
def handle_http_get(req, conn):
    ...
    if req.path == "/status":
        ...
        resp = handle_http_get_topics(req, conn)
    elif req.path.startswith("/whisper/feed/"):
        resp = handle_httpe_get_tweets(req, conn)
    else:
        resp = handle_http_get_file(req.path)
    return resp

def handle_http_get_topics(req, conn): #sidebar
...
def handle_httpe_get_tweets(req, conn):
    log("Handling http get tweets request")
    topic = req.path.split('/')[3].split('?')[0]
    version = int (req.path.split('=')[1])
    pos = topicsList.index(topic)
    print(pos)
    print(tweetList[pos])
...
def handle_http_post(req, conn):
    ...
    if req.path == "/whisper/messages":
        ...
        topics = lines[0][8:].strip().split()  # List of topics from the first line
        message = lines[1][11:].strip()  # Message content from the second line
        
        if not topics:
            topics = ["whatever"]  # Default topic if none specified
        if not message:
            send_http_response(conn, Response(200, "text/plain", "Empty messages are ignored"))
            return
    ...
    # Process POST /whisper/like/TOPIC
        ...
        topic = req.path[len("/whisper/like/"):]
